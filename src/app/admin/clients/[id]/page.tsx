  ...
  const handleSync = async (acc: AdAccount) => {
    ...
    try {
      const res = await fetch('/api/google/sync', {
        ...
        body: JSON.stringify({ adAccountId: acc.id, customerId: acc.accountId })
      })
      const data = await res.json()
      if (data.ok) showToast('✓ Sync: ' + data.synced + ' днів синхронізовано', 'ok')
      else showToast(data.error ?? 'Помилка sync', 'err')
    } catch {
  ...
  return (
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', background:'var(--bg)' }}>
      ...
      <main style={{ flex:1, overflowY:'auto' }}>
        ...
        <div style={{ maxWidth:'860px', margin:'0 auto', padding:'16px 16px 100px', position:'relative', zIndex:1 }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                        ...
                        {acc.platform === 'GOOGLE' && (
                          <button onClick={()=>handleSync(acc)} disabled={syncing===acc.id}
                            ...
                            style={{ display:'flex', alignItems:'center', gap:'6px', padding:'8px 14px', background:'rgba(230,0,0,0.08)', border:'1px solid rgba(230,0,0,0.2)', borderRadius:'7px', color:'#e60000', fontSize:'12px', fontWeight:600, cursor:'pointer', opacity: syncing===acc.id ? 0.5 : 1 }}
                          >
                            {syncing===acc.id ? '...' : '↻ Sync'}
                          </button>
                        )}
