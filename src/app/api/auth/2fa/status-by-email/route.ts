...
export async function POST(req: NextRequest) {
  ...
  if (blocked) return NextResponse.json({ error: `Акаунт тимчасово заблоковано. Спробуйте через ${minsLeft} хв.` })
  const user = await prisma.user.findUnique({ where: { email: emailLower } })
  if (!user?.password) { await loginAttempts.recordFail(emailLower); return NextResponse.json({ error: 'Невірний email або пароль' }) }
  ...
  if (!isValid) { await loginAttempts.recordFail(emailLower); return NextResponse.json({ error: 'Невірний email або пароль' }) }
