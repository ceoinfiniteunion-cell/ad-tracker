...
export function Sidebar() {
  ...
  return (
    <>
      {/* ── МОБІЛЬНИЙ ТОПБАР (CSS ховає на десктопі) ── */}
      <div data-mobile-topbar="" style={{ position:'fixed', top:0, left:0, right:0, height:'56px', background:'var(--bg2)', borderBottom:'1px solid var(--border)', alignItems:'center', justifyContent:'space-between', padding:'0 16px', zIndex:1000 }}>
      ...
      {/* ── SPACER для топбара (CSS ховає на десктопі) ── */}
      <div data-mobile-spacer="" style={{ flexShrink:0 }}/>

      {/* ── ДЕСКТОПНИЙ САЙДБАР (CSS ховає на мобільному) ── */}
      <aside style={{ width: collapsed ? '64px' : '220px', minWidth: collapsed ? '64px' : '220px', background:'var(--bg2)', borderRight:'1px solid var(--border2)', boxShadow:'2px 0 12px rgba(0,0,0,0.15)', display:'flex', flexDirection:'column', height:'100vh', position:'sticky', top:0, transition:'width 0.25s ease, min-width 0.25s ease', overflow:'hidden' }}>
