...
export async function getAccountInsights(accountId: string, from: string, to: string) {
  const data = await metaFetch(`/${accountId}/insights`, {
    fields: 'spend,impressions,clicks,actions,action_values,ctr,cpc,cpp,reach,frequency,unique_clicks,cost_per_unique_click',
...
export function parseConversions(actions: any[]): number {
  if (!actions) return 0
  const types = ['purchase','lead','complete_registration','add_to_cart','offsite_conversion.fb_pixel_purchase','offsite_conversion.fb_pixel_lead']
...
export function parseLeads(actions: any[]): number {
  if (!actions) return 0
  return actions.filter(a => a.action_type==='lead'||a.action_type==='offsite_conversion.fb_pixel_lead')
    .reduce((s, a) => s + Number(a.value), 0)
}

export function parseCostPerResult(costPerAction: any[]): number {
  if (!costPerAction) return 0
  const item = costPerAction.find(a => a.action_type==='lead'||a.action_type==='purchase')
